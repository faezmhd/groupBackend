...
class GetWishList(generics.ListAPIView):
    serializers_class = serializers.WishListSerializers
...
class ToggleWishList(APIView):
   ...
   def get(self, request):
       ...
       if not user_id or not product_id:
           return Response({'message': 'Invalid Request a user id and product id required'}, status=status.HTTP_400_BAD_REQUEST)
       ...
       wishlist_item , created = models.WishList.objects.get_or_create(user_id = request.user, product = product)
